...
bool BufferPoolManagerInstance::FlushPgImp(page_id_t page_id) {
  ...
  if(pages_[page_table_[page_id]].page_id_ == INVALID_PAGE_ID){
    latch_.unlock();
    return false;
  }
...
void BufferPoolManagerInstance::FlushAllPgsImp() {
  // You can do it!
  latch_.lock();
  for(auto &ipage : page_table_){
    if(pages_[ipage.second].page_id_ != INVALID_PAGE_ID && pages_[ipage.second].IsDirty()){
      disk_manager_->WritePage(pages_[ipage.second].page_id_, pages_[ipage.second].GetData());
    }
  }
  latch_.unlock();
}
...
Page *BufferPoolManagerInstance::NewPgImp(page_id_t *page_id) {
  ...
  // LOG_INFO("%d", (int)free_list_.size());
  page_id_t new_page_id = AllocatePage();
  *page_id = new_page_id;
  if(pages_[new_frame_id].IsDirty()){
    disk_manager_->WritePage(pages_[new_frame_id].GetPageId(), pages_[new_frame_id].data_);
  }
...
Page *BufferPoolManagerInstance::FetchPgImp(page_id_t page_id) {
  ...
  if(pages_[free_frame].IsDirty()){
    disk_manager_->WritePage(pages_[free_frame].GetPageId(), pages_[free_frame].data_);
  }

  page_table_.erase(pages_[free_frame].GetPageId());
